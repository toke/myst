...
source=(http://dl.suckless.org/st/$appname-$pkgver.tar.gz
        http://st.suckless.org/patches/st-externalpipe-$pkgver.diff
        http://st.suckless.org/patches/st-no_bold_colors-$pkgver.diff
        http://st.suckless.org/patches/st-solarized-dark-$pkgver.diff
...
md5sums=('1a926f450b4eacb7e2f5ac5b8ffea7c8'
         ...
         'b0fd749bd9aff7a286bd5618ba386f45')

build() {
    cd $srcdir/$appname-$pkgver
    patch -i $srcdir/st-inconsolata-dz-for-powerline-aa.diff
    patch -i $srcdir/st-externalpipe-$pkgver.diff
